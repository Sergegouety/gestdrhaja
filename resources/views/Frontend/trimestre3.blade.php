...
<form class="form-horizontal m-t-10 p-20 p-b-0" method="post" enctype="multipart/form-data" action="{{ route('update.ptab.trim') }}">
  ...
  <input type="hidden" name="trimestre" id="trimestre" value="3">
  <div class="modal-footer"></div>
                 <div class="row form-group" >
                    <label class="col-sm-2 col-sm-2 control-label" for="cible_t3">Cible 3eme Trim :</label>
                    <div class="col-sm-4">
                    <input type="text" class="form-control" name="cible_t3" id="cible_t3" value="{{$ptab->cible_t3}}" <?php if($isptabadmin==1 || $isptabHelper==1){echo '';}else{echo 'readonly';} ?>  >
                  </div>
                  
                  <label class="col-sm-2 col-sm-2 control-label">Coût 3ème Trim:</label>
                  <div class="col-sm-4">
                    <input type="text" class="form-control" name="cout_t3" id="cout_t3" value="{{$ptab->cout_t3}}" <?php if($isptabadmin==1 || $isptabHelper==1){echo '';}else{echo 'readonly';} ?>>
                  </div>

                </div>

                <div class="row form-group">
                    <label class="col-sm-2 col-sm-2 control-label" for="valeur_t3" >Valeur Trim 3 : 
                      <a id="add-without-image1" class="badge bg-warning" href="javascript:;">?</a>
                    </label>

                    <div class="col-sm-4" >
                    <textarea class="form-control" name="valeur_t3" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly(3,$grade,$ptab->cible_t3,$ptab->user_id); } ?> >{{$ptab->valeur_t3}}</textarea>
                  </div>
...
<div class="modal-footer"></div>
                 <div class="modal-body ace-scrollbar">
                    ...
                    <div class="mb-1 mb-sm-0 col-sm-2 col-form-label text-sm-right pr-0">
                      <button type="button" onclick="addElement3()" class="btn btn-success radius-round">
                             <span class="fa fa-plus"></span>
                      </button>
                    </div>

                    <div class="col-sm-4 input-floating-label text-blue-d2 brc-blue-m1">
                       <input type="file" class="form-control" name="livrable_t3[]" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables(3,$grade,$ptab->cible_t3,$ptab->user_id);} ?> >
                    ...
                    <div class="col-sm-6 input-floating-label text-blue-d2 brc-blue-m1">
                      <textarea class="form-control form-control-lg col-sm-12 col-md-10 shadow-none" id="id-form-field-2" name="commentaire_lt3[]" placeholder="Commentaire" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly(3,$grade,$ptab->cible_t3,$ptab->user_id); } ?>></textarea>
                    </div>
                  </div>
                  <div id="link_section3" >
 ...
 @php 
        ...
        if($ptab->user_id == Auth::user()->id){ $autre_livrableT=getOtherLivrable($ptab->id,3,$type_id);  }
    else{$autre_livrableT=getOtherLivrableHierachie($ptab->id,3,$type_id); }

        $autre_livrable=getOtherLivrableState($ptab->id,3,$type_id,1); 
        $i = 1;@endphp
<div class="modal-footer"></div>
<table class="table table-striped table-advance table-hover">
<h4 class="mb">Liste des livrables du 3ème trimestre</h4>
@if(count($autre_livrable) && $ptab->user_id == Auth::user()->id)
 <a data-toggle="modal" data-target="#envoiModal3" class="btn btn-success" id="ajouter">
      ...
      <tbody class="pos-rel">
        ...
        <tr class="d-style bgc-h-orange-l4">
            ...
            <td>
              ...
              @if(count($autre_livrable))
              <button data-toggle="modal" data-target="#modifModal3" class="btn btn-primary btn-xs" onclick="modifLivrable({{$livrable->id}},'{{addslashes($livrable->commentaire)}}',3)"><i class="fa fa-pencil"></i></button>
...
<div class="modal-footer"></div>
<table class="table table-striped table-advance table-hover">
<h4 class="mb">Liste des Statuts des livrables du 3ème trimestre</h4>
  <thead class="sticky-nav text-green-m1 text-uppercase text-85" >
      ...
      <tbody class="pos-rel">
        @php $autre_comments=getOtherCommentaire($ptab->id,3,$ptab->type_id);  $j = 1;@endphp
...
<form class="form-horizontal m-t-10 p-20 p-b-0" method="post" enctype="multipart/form-data" action="{{ route('update.ptab.commentaire') }}">
  {{ csrf_field() }} 
                ...
                <div class="row form-group">
                    ...
                    <div class="col-sm-2">
                     <select class="form-control" name="statut_t3" id="statut_t3" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(3,$grade,$ptab->cible_t3); } ?>>
                        <option value=""></option>
                      <option   value="2" <?php if($ptab->is_valide3=='2'){echo 'selected';} ?>>Valider</option>
                      <option   value="-1" <?php if($ptab->is_valide3=='-1'){echo 'selected';} ?>>Réjeter</option>
                    </select>
                    </div>
               <label class="col-sm-2 control-label">Etat d'avancement</label>
                    <div class="col-sm-3">
                     <select class="form-control" name="statut_avancement" id="statut_avancement" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(3,$grade,$ptab->cible_t3); } ?>  required > 
                        <option value=""></option>
                      <option <?php if($ptab->statut_t3=='3'){echo 'selected';} ?>  value="3">Réalisé</option>
                      <option <?php if($ptab->statut_t3=='2'){echo 'selected';} ?>  value="2">Partiellement Réalisé</option>
                      <option <?php if($ptab->statut_t3=='1'){echo 'selected';} ?>  value="1">Non Réalisé</option>
                    </select>
                    </div>
                    </div>
                 ...
                 <label class="col-sm-3 control-label" for="observation_t1">Observation :</label>
                  <div class="col-sm-8">
                    <textarea class="form-control" rows="5" name="observation_t3" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly2(3,$grade,$ptab->cible_t3); } ?> >{{$ptab->observation_t3}}</textarea>
                </div>
              </div>
@if($isptabadmin || ptab_gestion_rigth(Auth::user()->id))
              <div class="form-group">
                <label class="col-sm-2 control-label">Statut final</label>
                    <div class="col-sm-2">
                     <select class="form-control" name="statut_final" id="statut_final" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(3,$grade,$ptab->cible_t3); } ?>>
                        <option value=""></option>
                      <option <?php if($ptab->statut_t3=='3'){echo 'selected';} ?>  value="3">Réalisé</option>
                      <option <?php if($ptab->statut_t3=='2'){echo 'selected';} ?>  value="2">Partiellement Réalisé</option>
                      <option <?php if($ptab->statut_t3=='1'){echo 'selected';} ?>  value="1">Non Réalisé</option>
                    ...
                    <div class="col-sm-6">
                      <textarea class="form-control" rows="5" name="commentaire_final" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly2(3,$grade,$ptab->cible_t3); } ?> >{{$ptab->commentaire_t3}}</textarea>
...
@endif
        ...
        <div class="modal fade" id="envoiModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<div class="modal fade" id="modifModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              ...
              <div class="modal-body">
                ...
                <form class="form-horizontal m-t-10 p-20 p-b-0" method="post" action="{{route('modif.livrable')}}" enctype="multipart/form-data" id="arret_form">
                     {{ csrf_field() }} 
                     <input type="hidden" name="livrable_id3" id="livrable_id3">
                   ...
                   <div class="form-group">
                    <label class="col-sm-4 col-sm-4 control-label">livrables/pièces justificatives </span></label>
                        <div class="col-sm-8 input-floating-label text-blue-d2 brc-blue-m1">
                               <input type="file" class="form-control" name="livrable_t3" >
                               <span class="floating-label text-grey-m3">
                    ...
                    <div class="form-group">
                      <label class="col-sm-4 col-sm-4 control-label">Commentaire</label>
                      <div class="col-sm-8">
                        <textarea class="form-control" rows="5" name="liv_com3" id="liv_com3" ></textarea>
                      </div>
