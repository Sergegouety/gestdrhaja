  ...
  <div class="row mt">
          <div class="col-lg-12">
            ...
            <div class="form-panel">
              <h4 class="mb"> Modifier Action</h4>
               @if (session('success'))
            ...
            <div class="row">
                ...
                <div class="modal-body ace-scrollbar">
                 <form class="form-horizontal m-t-10 p-20 p-b-0" method="post" action="{{ route('update.ptab.detail') }}" id="form_action">
                  ...
                  <div class="form-group">
                    <label class="col-sm-2 col-sm-2 control-label" style="padding-top: 10px;">Axe :</label>
                     <div class="col-sm-10" style="padding-top: 10px;">
                      <select class="form-control" name="axe" id="axe" onchange="getExtrant(this.value)" required="">
                        <option></option>
                         @foreach($axes as $axe)
                          <option value="{{$axe->id}}" <?php if($axe->id == $ptab->axeId){ echo 'selected';} ?> >{{ $axe->id.' : '.$axe->axe}}</option>
                         @endforeach
                    </select>
                    </div>

                    <label class="col-sm-2 col-sm-2 control-label" style="padding-top: 10px;">Extrant :</label>
                     <div class="col-sm-10" style="padding-top: 10px;">
                     <select class="form-control" name="extrant" id="extrant" onchange="getAction(this.value)" required>
                        @foreach($extrants as $extrant)
                          <option value="{{$extrant->id}}" <?php if($extrant->id == $ptab->extrantId){ echo 'selected';} ?> >{{ $extrant->ref.' : '.$extrant->extrant}}</option>
                         @endforeach
                      </select>
                    </div>

                    <label class="col-sm-2 col-sm-2 control-label" style="padding-top: 10px;">Action :</label>
                     <div class="col-sm-10" style="padding-top: 10px;">
                     <select class="form-control" name="master_action_id" id="master_action_id" onchange="getIntituleIndicateur(this.value)" required>
                         @foreach($master_actions as $master_action)
                          <option value="{{$master_action->id}}" <?php if($master_action->id == $ptab->reference_matrice){ echo 'selected';} ?> >{{ $master_action->ref.' : '.$master_action->intitule_action}}</option>
                         @endforeach
                         <option value="new">Autre action</option>
                      </select>
                    </div>

                   <label class="col-sm-2 col-sm-2 control-label" style="padding-top: 10px;">Intitulé :</label>
                    <div class="col-sm-10" style="padding-top: 10px;">
                      <textarea class="form-control" rows="3" name="intitule" id="intitule" readonly >{{getInstanceName('action','id',$ptab->id,'intitule')}}</textarea>
                    </div>

                    <label class="col-sm-2 col-sm-2 control-label" style="padding-top: 10px;">Indicateur :</label>
                    <div class="col-sm-10" style="padding-top: 10px;">
                      <textarea class="form-control" rows="3" name="indicateur" id="indicateur" readonly >{{getInstanceName('action','id',$ptab->id,'indicateur')}}</textarea>
                    </div>
