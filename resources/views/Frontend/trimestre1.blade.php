...
<form class="form-horizontal m-t-10 p-20 p-b-0" method="post" enctype="multipart/form-data" action="{{ route('update.ptab.trim') }}">
  ...
  <input type="hidden" name="trimestre" id="trimestre" value="1">
  <div class="modal-footer"></div>
                 <div class="row form-group" >
                    <label class="col-sm-2 col-sm-2 control-label" for="cible_t1">Cible 1er Trim :</label>
                    <div class="col-sm-4">
                    <input type="text" class="form-control" name="cible_t1" id="cible_t1" value="{{$ptab->cible_t1}}" <?php if($isptabadmin==1 || $isptabHelper==1){echo '';}else{echo 'readonly';} ?>  >
                  </div>
                  
                  <label class="col-sm-2 col-sm-2 control-label">Coût 1er Trim:</label>
                  <div class="col-sm-4">
                    <input type="text" class="form-control" name="cout_t1" id="cout_t1" value="{{$ptab->cout_t1}}" <?php if($isptabadmin==1 || $isptabHelper==1){echo '';}else{echo 'readonly';} ?>>
                  </div>

                </div>

                <div class="row form-group">
                    <label class="col-sm-2 col-sm-2 control-label" for="valeur_t1" >Valeur Trim 1 : 
                      <a id="add-without-image1" class="badge bg-warning" href="javascript:;">?</a>
                    </label>

                    <div class="col-sm-4" >
                    <textarea class="form-control" name="valeur_t1" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly(1,$grade,$ptab->cible_t1,$ptab->user_id); } ?> >{{$ptab->valeur_t1}}</textarea>
                  </div>
...
<div class="modal-footer"></div>
                 <div class="modal-body ace-scrollbar">
                    ...
                    <div class="col-sm-4 input-floating-label text-blue-d2 brc-blue-m1">
                       <input type="file" class="form-control" name="livrable_t1[]" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables(1,$grade,$ptab->cible_t1,$ptab->user_id);} ?> >
                    ...
                    <div class="col-sm-6 input-floating-label text-blue-d2 brc-blue-m1">
                      <textarea class="form-control form-control-lg col-sm-12 col-md-10 shadow-none" id="id-form-field-2" name="commentaire_lt1[]" placeholder="Commentaire" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly(1,$grade,$ptab->cible_t1,$ptab->user_id); } ?>></textarea>
                    </div>
                  </div>
                  <div id="link_section" >
 ...
 @php 
    ...
    $autre_livrableT=getOtherLivrable($ptab->id,1,$type_id); 
   
    if($ptab->user_id == Auth::user()->id){ $autre_livrableT=getOtherLivrable($ptab->id,1,$type_id);  }
    else{$autre_livrableT=getOtherLivrableHierachie($ptab->id,1,$type_id); }

    $autre_livrable=getOtherLivrableState($ptab->id,1,$type_id,1);  
    $i = 1;
    @endphp

<div class="modal-footer"></div>
<table class="table table-striped table-advance table-hover">
<h4 class="mb">Liste des livrables du 1er trimestre</h4>
@if(count($autre_livrable) && $ptab->user_id == Auth::user()->id)
 <a data-toggle="modal" data-target="#envoiModal" class="btn btn-success">
      ...
      <tbody class="pos-rel">
        ...
        <tr class="d-style bgc-h-orange-l4">
            ...
            <td>
              ...
              <button data-toggle="modal" data-target="#modifModal" class="btn btn-primary btn-xs" onclick="modifLivrable({{$livrable->id}},'{{addslashes($livrable->commentaire)}}',1)"><i class="fa fa-pencil"></i></button>
...
<div class="modal-footer"></div>
<table class="table table-striped table-advance table-hover">
<h4 class="mb">Liste des Statuts des livrables du 1er trimestre</h4>
  <thead class="sticky-nav text-green-m1 text-uppercase text-85" >
      ...
      <tbody class="pos-rel">
        @php $autre_comments=getOtherCommentaire($ptab->id,1,$ptab->type_id);  $j = 1;@endphp
...
<form class="form-horizontal m-t-10 p-20 p-b-0" method="post" enctype="multipart/form-data" action="{{ route('update.ptab.commentaire') }}">
  {{ csrf_field() }} 
                ...
                <input type="hidden" name="trimestre" id="trimestre" value="1">
                <div class="row form-group">
                    <label class="col-sm-3 control-label">Statut livrable /pièces justificatives :</label>
                    <div class="col-sm-3">
                     <select class="form-control" name="statut_t1" id="statut_t1" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(1,$grade,$ptab->cible_t1); } ?>>
                        <option value=""></option>
                      <option   value="2" <?php if($ptab->is_valide1=='2'){echo 'selected';} ?>>Valider</option>
                      <option   value="-1" <?php if($ptab->is_valide1=='-1'){echo 'selected';} ?>>Réjeter</option>
                    </select>
                    </div>
                    <label class="col-sm-2 control-label">Etat d'avancement</label>
                    <div class="col-sm-3">
                     <select class="form-control" name="statut_avancement" id="statut_avancement" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(1,$grade,$ptab->cible_t1); } ?>  required > 
                        <option value=""></option>
                 ...
                 <label class="col-sm-3 control-label" for="observation_t1">Observation :</label>
                  <div class="col-sm-8">
                    <textarea class="form-control" rows="5" name="observation_t1" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly2(1,$grade,$ptab->cible_t1); } ?> >{{$ptab->observation_t1}}</textarea>
                </div>
              </div>
@if($isptabadmin || ptab_gestion_rigth(Auth::user()->id))
              <div class="form-group">
                <label class="col-sm-2 control-label">Statut final</label>
                    <div class="col-sm-2">
                     <select class="form-control" name="statut_final" id="statut_final" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_disables2(1,$grade,$ptab->cible_t1); } ?>>
                        <option value=""></option>
                      <option <?php if($ptab->statut_t1=='3'){echo 'selected';} ?>  value="3">Réalisé</option>
                      <option <?php if($ptab->statut_t1=='2'){echo 'selected';} ?>  value="2">Partiellement Réalisé</option>
                      <option <?php if($ptab->statut_t1=='1'){echo 'selected';} ?>  value="1">Non Réalisé</option>
                    ...
                    <div class="col-sm-6">
                      <textarea class="form-control" rows="5" name="commentaire_final" <?php if($isptabadmin!=1 && $isptabHelper!=1){ echo_readonly2(1,$grade,$ptab->cible_t1); } ?> >{{$ptab->commentaire_t1}}</textarea>
...
<div class="modal fade" id="envoiModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <form class="form-horizontal m-t-10 p-20 p-b-0" method="post" action="{{route('modif.livrable')}}" enctype="multipart/form-data" id="arret_form">
                     {{ csrf_field() }} 
                     <input type="hidden" name="livrable_id" id="livrable_id">
                   ...
                   <div class="form-group">
                    <label class="col-sm-4 col-sm-4 control-label">livrables/pièces justificatives </span></label>
                        <div class="col-sm-8 input-floating-label text-blue-d2 brc-blue-m1">
                               <input type="file" class="form-control" name="livrable_t1" >
                               <span class="floating-label text-grey-m3">
                    ...
                    <div class="form-group">
                      <label class="col-sm-4 col-sm-4 control-label">Commentaire</label>
                      <div class="col-sm-8">
                        <textarea class="form-control" rows="5" name="liv_com" id="liv_com" ></textarea>
                      </div>
